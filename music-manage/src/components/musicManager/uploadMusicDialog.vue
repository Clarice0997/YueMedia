<template>
  <!-- 新增音乐对话框 -->
  <el-dialog title="新增歌曲" :visible.sync="localDialogInsertMusicFormVisible" append-to-body
             @close="resetUploadMusicForm">
    <el-form ref="insertMusicForm" :model="insertMusicForm" :rules="insertMusicFormRules" label-position="top">
      ...
      <el-form-item label="上传音乐文件" prop="musicFile">
        <el-upload
          ...
          :action=`${process.env.VUE_APP_REQUEST_URL}/apis/music/upload/music`
        >
      ...
      <el-form-item label="上传封面文件" prop="coverFile">
        <div style="width: 148px; height: 148px; display: inline-block; border: 1px dashed #c0ccda; border-radius: 6px">
          <el-image :src="coverImage" style="width: 148px; height: 148px">
            <div slot="error" class="image-slot"
                 style="width: 148px; height: 148px; display: flex; align-items: center; justify-content: center">
        ...
        <el-upload
          ...
          with-credentials
          :headers="{ Authorization }"
          :action=`${process.env.VUE_APP_REQUEST_URL}/apis/music/upload/music/cover`
      ...
      <el-form-item label="歌曲名称" prop="songName">
        <el-input v-model="insertMusicForm.songName" placeholder="输入歌曲名称" clearable
                  :disabled="!isMusicUploaded"></el-input>
      </el-form-item>
      <!-- 歌手名 -->
      <el-form-item label="歌手名" prop="singerName">
        <el-input v-model="insertMusicForm.singerName" placeholder="输入歌手名" clearable
                  :disabled="!isMusicUploaded"></el-input>
      </el-form-item>
      <!-- 专辑名 -->
      <el-form-item label="专辑名" prop="albumName">
        <el-input v-model="insertMusicForm.albumName" placeholder="输入歌曲专辑名" clearable
                  :disabled="!isMusicUploaded"></el-input>
      </el-form-item>
      <!-- 歌曲年份 -->
      <el-form-item label="歌曲年份" prop="year">
        <el-date-picker v-model="insertMusicForm.year" type="year" format="yyyy" value-format="timestamp"
                        :disabled="!isMusicUploaded" placeholder="选择歌曲年份"></el-date-picker>
      </el-form-item>
    </el-form>
    <div slot="footer" class="dialog-footer">
      <el-button @click="clickCloseInsertMusicDialogHandler">取 消</el-button>
      <el-button type="warning" :disabled="!isMusicUploaded" v-loading.fullscreen="fullscreenLoading"
                 :loading="resetBtnLoading" @click="clickResetInsertMusicHandler">重 置
      </el-button>
      <el-button type="primary" :disabled="!isMusicUploaded" v-loading.fullscreen="fullscreenLoading"
                 :loading="uploadBtnLoading" @click="clickConfirmInsertMusicHandler">确 定
      </el-button>
    </div>
  </el-dialog>
</template>
