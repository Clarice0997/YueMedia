<template>
  <div>
    <el-table ref="targetedErrorTable" :data="currentTargetedErrorTableData" :height="400" :max-height="400" tooltip-effect="dark" :row-class-name="tableRowClassName" class="targetedErrorTable">
      ...
      <el-table-column label="错误类型" align="center" width="100" prop="name"></el-table-column>
      <el-table-column label="错误日期" align="center" width="250" sortable>
        <template slot-scope="scope">
      ...
      <el-table-column label="操作" fixed="right" width="300" align="center">
        <template slot-scope="scope">
          <el-button type="success" @click="clickStatusChangeHandler(scope.row, 2)">已处理</el-button>
        </template>
      </el-table-column>
      <!--  数据为空插槽  -->
      <template slot="empty">
...
export default {
  name: 'targetedErrorTable',
  data() {
    return {
      targetedErrorTableData: [],
      pageNumber: 1,
      pageSize: 5
    }
  },
  computed: {
    currentTargetedErrorTableData() {
      return this.targetedErrorTableData.slice((this.pageNumber - 1) * this.pageSize, (this.pageNumber - 1) * this.pageSize + this.pageSize)
    },
    totalData() {
      return this.targetedErrorTableData.length
    }
  },
  filters: {
  ...
  methods: {
    ...
    async initTableData() {
      const {
        data: { errors }
      } = await getErrorsAPI(2)
      this.targetedErrorTableData = errors
    },
    // 更新表格数据
